<template>
  <div class="trade-container">
    ...
    <div class="trade">
      <div class="price">
        应付金额:　<span>¥{{ totalInfo.price }}</span>
      </div>
    ...
    <div class="sub clearFix">
      <a class="subBtn" @click.prevent="submit">提交订单</a>
    </div>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    const totalInfo = computed(() => {
      let price = 0,
        num = 0;
      detailArrayList.value.forEach((item) => {
    ...
    const tradeNo = computed(() => {
      return store.getters.tradeNo;
    });
    function submit() {
      let data = {
        consignee: "张飞",
        consigneeTel: "13188888888",
        deliveryAddress: "大学",
        paymentWay: "ONLINE",
        orderComment: msg.value,
        orderDetailList: detailArrayList.value,
      };
      API.reqSubmitOrder(tradeNo.value, data).then((res) => {
        if (res.code === 200) {
          router.push({ path: "/pay", query: { orderId: res.data } });
        } else {
          alert(res.message);
        }
      });
